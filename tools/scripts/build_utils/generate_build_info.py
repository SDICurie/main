...
def main():
    ...
    # Hardcoded value to be consistent in the debug toolchain (dump, crashtool, jenkins)
    previousJson['project'] = "curie_reference"
    ...
    previousJson['variants'][variant]['package_file'] = '/device/'+project+'/image/'+board+'/'+buildvariant+'/'+variant+'-'+build_tag+'.zip'
    ...
    previousJson['hardwares'][board]['variants'][project+'-'+buildvariant] = {'hardware_family': project, 'flashfiles': {'flash': ['/device/'+project+'/image/'+board+'/'+buildvariant+'/'+variant+'-'+build_tag+'.zip:flash.json']}}
